    ...
    StudentLogin.find({studentid:req.body.loginid,"grades.courseid":req.body.id}, {_id:0, grades: 1}, (err, results) => {
      if(err){s
        ...
        arr.forEach(function(grade){
            ...
            // })
            courseresult.push({studentid:req.body.loginid,courseid:grade.courseid,assignmentid:grade.assignmentid,"score":grade.score})
              ...
              arr2.forEach(function(studentres){
                  ...
                  StudentLogin.find({studentid:studentres.studentid,"grades.courseid":req.body.id}, {_id:0, grades: 1}, (err, results) => {
                    if(err){
                      ...
                      console.log("Successfully retrieved grades");
                      if(results.length>0){
                      console.log("results",results[0].grades)
                      // res.end(JSON.stringify(results[0].grades));
                      if(results.length>0){
                      ...
                      arr.forEach(function(grade){
                        ...
                        // counter1++;
                        //   })
                                   courseresult.push({studentid:studentres.studentid,courseid:grade.courseid,score:grade.score,assignmentid:grade.assignmentid})
