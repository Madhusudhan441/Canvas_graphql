...
export default class Assignment_det extends Component {
    constructor(props) {
        ...
        this.state={
            ...
            clickid:localStorage.getItem('loginid')
        }
...
this.setState({
    ...
    viewdownload=(e)=>{
      ...
      axios.get('http://localhost:3001/downloadassign',{
        params: {
          file: '/submissions/'+localStorage.get('courseid')+'/'+localStorage.getItem('loginid')+'/'+localStorage.getItem('assignmentid')+this.state.filename
    ...
    viewAssignment(value){
      ...
      axios.get('http://localhost:3001/seeFiles',{params:{courseid:localStorage.getItem('courseid'),stufac:"student",studentid:value,assignmentid:localStorage.getItem('assignmentid')}},).
      then(response => {
              console.log("in then")
              // alert("in then")
              console.log(response.data)
    ...
    handleDownload(value) {
      ...
      axios.post(url,{data:{courseid:localStorage.getItem('courseid'),studentid:this.state.clickid,assignmentid:localStorage.getItem('assignmentid')}},{
      }).then(response=>{
    ...
    componentDidMount(){
      ...
      const data_stud={
        ...
        axios.post('http://localhost:3001/getpeople',data_stud)
        .then((response) => {
          if(response.status === 200){
            this.setState({
...
// if(localStorage.getItem('stufac')=="student"){
  // alert("this")
      ...
      axios.get('http://localhost:3001/seeFiles',{params:{courseid:localStorage.getItem('courseid'),stufac:localStorage.getItem('stufac'),studentid:localStorage.getItem('loginid'),assignmentid:localStorage.getItem('assignmentid')}},).
      then(response => {
              console.log("in then")
              console.log(response.data)
...
// }
  ...
  render() {
    ...
    if(this.state.studentlist.length>0){
      ...
      return(
      <div >
        <a onClick={this.viewAssignment.bind(this,student.studentid)}>{student.studentid}</a>
      </div>
      )
      })
    }
